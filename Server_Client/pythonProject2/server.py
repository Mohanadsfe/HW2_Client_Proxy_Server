...
def server(host: str, port: int) -> None:
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as server_socket:  # פתיחת סוקיט של השרת
        ...
        server_socket.bind((host, port))  # get the server socket the host and port that should listening on.
        server_socket.listen(1)  # here the server waiting to get any requests
...
def client_handler(client_socket: socket.socket, client_address: tuple[str, int]) -> None:
    ...
    with client_socket:  # closes the socket when the block is exited
        ...
        while True:
            # לקבל את המידע מהשרת
            # get the data from the client with max size api.BUFFER_SIZE (data = request)
            data = client_socket.recv(api.BUFFER_SIZE)  # * Fill in start (3) # * Fill in end (3)
            if not data:
                break
            try:

                try:  # קריאה של הבקשה מהלקוח
                ...
                # After handling with the request , send a response of it.
